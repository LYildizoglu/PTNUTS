classdef PestoSamplingOptions < matlab.mixin.CustomDisplay
   ...
   properties
      ...
      % Langevin algorithm), 'NUTS' (No-U-Turn Sampler), 'PTNUTS' 
      % (parallel tempered No-U-Turn Sampler), or 'DRAM' 
      % (delayed rejection adapted Metropolis algorithm, only if the DRAM toolbox is installed).
      % Default value is 'PT'
      ...
      % No-U-Turn Options options, an instance of
      % NUTSOptions
      NUTS; 

      % parallel tempered No-U-Turn Sampler options, an instance of
      % PTNUTSOptions
      PTNUTS; 
   ...
   methods
      ...
      function this = set.samplingAlgorithm(this, value)
         ...
         if (strcmp(value, 'MALA') || strcmp(value, 'DRAM') || strcmp(value, 'PT') || strcmp(value, 'PHS') ...
               || strcmp(value, 'RAMPART') || strcmp(value, 'NUTS') || strcmp(value, 'PTNUTS'))
      ...
      function this = checkDependentDefaults(this, par)
         ...
         if ~isempty(this.theta0)
            switch this.samplingAlgorithm
               ...
               case 'PTNUTS'
                  if size(this.theta0,1) ~= par.number || ...
                        (size(this.theta0,2) ~= this.PTNUTS.nTemps && size(this.theta0,2) ~= 1)
                     error('Please make sure opt.theta0, the par.number and opt.PTNUTS.nTemps are consistent.')
                  end
               case 'PHS'
                  if size(this.theta0,1) ~= par.number || ...
            ...
            switch this.samplingAlgorithm
               ...
               case 'PTNUTS'
                  this.theta0 = bsxfun(@plus, par.min, ...
                     bsxfun(@times, par.max - par.min, rand(par.number,this.PTNUTS.nTemps)));
               case 'PHS'
                  this.theta0 = bsxfun(@plus, par.min, ...
         ...
         if ~isempty(this.sigma0)
            %NUTS and PTNUTS do not use sigma, so no case here
            switch this.samplingAlgorithm
               case {'DRAM','MALA'}
