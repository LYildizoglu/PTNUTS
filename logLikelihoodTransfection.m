function [logL, dlogLdtheta, FIM] = logLikelihoodTransfection(varargin)
...
% State of the system:
% X = [X_1, X_2] with
% X_2 = [G]: eGFP concentration
% X_1 = [m]: mRNA concentration
%
% Dynamics:
% dX_2/dt = k_TL * X_1 - beta * X_2
% dX_1/dt = -delta * X_1
%
% Measurement:
% Y = X_2
...
% Log-likelihood
logL = - 1/2 * nansum(log(2*pi*sigma^2) + ((D-Y) ./ sigma).^2);

% Gradient
dlogLdtheta(1:4,1) = 1/(sigma^2) * nansum(bsxfun(@times,(D-Y), SY))';
dlogLdtheta(5,1)   = - nansum(1/sigma - (D-Y).^2 ./ (sigma^3)) * (10.^theta(5) * log(10));
