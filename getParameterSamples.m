function parameters = getParameterSamples(parameters, objFkt, options)
   ...
   switch options.MCMC.samplingAlgorithm
      ...
      % NUTS
      case 'NUTS'
         parameters.S = performNUTS( logPosterior, parameters, options.MCMC );

      % PTNUTS
      case 'PTNUTS'
         parameters.S = performPTNUTS( logPosterior, parameters, options.MCMC );
